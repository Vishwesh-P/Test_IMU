...
float thetaM, phiM, phiF, thetaF, phiFOld = 0, thetaFOld = 0;
float LPF = 0.8, LPF1 = 0.85;
float acc_3D;
...
void loop(void)
{
  ...
  thetaM = acos(AX/acc_3D);
  thetaF = thetaFOld*LPF + thetaM*(1-LPF);
  thetaFOld = thetaF;
  
  ///// POSI is top view angle wrt Z showing direction  
  phiM = atan2(AY,AZ); // wrt Z
  phiF = phiFOld*LPF1 + phiM*(1-LPF1);
  phiFOld = phiF;
    
  Serial.print(thetaM * 180 / PI);   Serial.print(",");
  Serial.print(thetaF * 180 / PI);   Serial.print(","); 
  Serial.print(phiM * 180 / PI);  Serial.print(",");
  Serial.print(phiF * 180 / PI);  Serial.print(",");  
